...
**Flags** (an independent JSON set): `stalled`, `orphaned`, `diverged`, `stale-contract`, `env-suspect`, `forced`, `block-malformed`, `outside-writes` (§12.3.3's comparator: something changed outside the isolation boundary since spawn — deliberately NOT `diverged`, which means the child committed outside its declared paths inside its own worktree), `seed-pending` (§11.4: a seed is durably owed to a task and has not been delivered; a delivery timeout is not a failure and never clears the seed), `conflict` (§11.4 step 2: materializing a producer's branch into a parked child's worktree hit a merge conflict; set *with* `seed-pending`, never instead of it), `first-check-green` / `first-check-red` (§2's M2 made durable — the first check's verdict for a task, written once and never again). The last four were added by the §§9-12 implementation; growing this list needs no migration because the decoder round-trips unknown flags on purpose, which is the property that lets the vocabulary grow at all. The M2 pair is **not telemetry**: nothing aggregates it across runs, §2 records the measurements per initiative in this spec's follow-up rather than in the DB, and the two flags are per-task state of one run that dies with the run. Two flags rather than one because with a single `first-check-green`, "never checked" and "checked and red" would both read as its absence and M2's denominator would silently become the task count. Keeping these out of the state column is deliberate: a state machine with a `stalled` state has to define the cross product of stalled × everything, and the transition table stops being testable. Flags are set and cleared independently and never gate a transition on their own.
