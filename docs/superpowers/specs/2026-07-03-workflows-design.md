...
6. **Advance is a compare-and-swap claim BEFORE any launch:**
   ...
   **§2.6 note (Task-2 adjudicated):** the CAS remains the sole gate on run-row mutation. For `continue`, the claim strictly precedes any action. For `fork`/`fresh`, `Launcher.Launch` mints its session id internally, so the order is Launch → SetTags → CAS — mirroring Start. A rejected CAS after a successful Launch (two-instance race) leaves one real, `wf:`-tagged session unrecorded on the run; the caller receives `ErrRunAdvancedElsewhere` naming it. Run-row integrity holds in all races; a launch FAILURE (the common case) is a clean no-op on the run.
