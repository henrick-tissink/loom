...
**Canonicalization is on writes only** — `filepath.Abs` + `Clean` for discovery output, folder-picker results, and `project_repos` inserts (a trailing slash otherwise breaks both arms of the match *and* mints a second PK row for the same directory). Symlinks are handled at the *comparison* site — each project root is resolved once per discovery pass and matched against both its raw and resolved forms, which is O(projects), not O(sessions).

**Session cwds ARE resolved, at launch.** Revision 2 stated the opposite — "never `EvalSymlinks` a stored cwd", reasoning that resolving would break transcript lookup — and `docs/spikes/2026-07-22-add-dir-spike.md` disproved it. A process's `getcwd()` returns the physical path, so claude records `/private/tmp/x` and derives its transcript **directory name** from that even when tmux was given `-c /tmp/x`. Storing the unresolved form makes `transcript.Path()` compute `-tmp-x` against a directory claude never wrote: the reader finds nothing, the session sits at `unknown` forever with no title and no context gauge, and workflow `{{prev.*}}` extraction silently yields empty. `session.physicalDir` therefore resolves cwd and add-dirs at `Launch`/`Resume` — after validation, so errors still name the path the user typed. This also makes the launch path agree with the adopt path, which already stored the physical form via `#{pane_current_path}`. (The adversarial review endorsed the wrong rule too; only the experiment caught it.)
...
**Spike — `docs/spikes/2026-07-22-add-dir-spike.md` — DONE, findings binding.** Outcomes:
- **Trust prompts once, for the primary cwd only.** `--add-dir` grants the sibling silently; writes to it succeed with no prompt. No per-add-dir trust handling is needed.
- **The shipped `DefaultTrustMarker` never matched this claude build** (v2.1.217 asks "Quick safety check: Is this a project you created or one you trust?"), so the trust gate was inert and the seed was unprotected. The version-independent `selectCursorPattern` is the real defence; the marker is now belt-and-braces only.
- **Transcripts key on the PHYSICAL cwd** — see §4, which this reversed.
- **`--resume` does not restore add-dirs** (inferred: the resumed write fell through to the auto-mode classifier where the original did not; the account default masked it). Loom re-passes them explicitly, so behaviour is already correct.
